...
f = open('sample/lstm.csv')
df = pd.read_csv(f)     # 读入流量数据
data = np.array(df['流量'])   # 获取流量序列
...
# ————————————————预测模型————————————————————
def prediction():
    pred, _ = lstm(x)      # 预测时只输入[1,time_step,input_size]的测试数据
    saver = tf.train.Saver(tf.global_variables())
    with tf.Session() as sess:
        # 参数恢复
        ...
        for step in range(test_size):
            prob = sess.run(pred, feed_dict={X: [train_x[step+train_size]]})
