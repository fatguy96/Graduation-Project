...
class My_SVR:
    def __init__(self, train_x, train_y, verify_x, verify_y, cv):
        self.train_x = train_x
        self.train_y = train_y
        self.verify_x = verify_x
        self.verify_y = verify_y
        self.n_folds = cv
    ...
    def train(self):
        ...
        train_sizes, train_scores_svr, test_scores_svr = \
            learning_curve(model_svr, self.train_x, self.train_y, train_sizes=np.linspace(0.1, 1, 10),
                           scoring="neg_mean_squared_error", cv=self.n_folds)
    ...
    def verify(self):
        ...
        acc = np.abs(self.verify_y-pre_y)/self.verify_y
        tmp_list.append('{}%'.format(acc*100))
        model_metrics_list.append(tmp_list)  # 将结果存入回归评估指标列表

        df2 = pd.DataFrame(model_metrics_list, index=['svr'], columns=['ev', 'mae', 'mse', 'r2', 'acc'])  # 建立回归指标的数据框
        ...
        num_epoch = int(self.verify_x/288)
