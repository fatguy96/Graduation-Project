...
def load_data(style, filename='sample/data.csv', shuffle=True):
    data = []
    i = 1
...
def train(filename):
    ...
    with tf.Session() as sess:
        ...
        saver.save(sess, 'model/%s' % names[-1][0:-4])
        matplotlib.pyplot.savefig('bp_loss/{}.png'.format(names[-1][0:-4]))
...
if __name__ == '__main__':
        ...
        if UI_filename != "":
            try:
                ...
                for i in range(epoch):
                    ...
                    matplotlib.pyplot.savefig('bp_test/test%d.png' % (i + 1))
                    matplotlib.pyplot.close()
    ...
    imgInfo = PhotoImage(file="bp_loss/login.png")
