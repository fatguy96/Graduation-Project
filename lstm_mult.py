...
lr = 0.0006         # 学习率
# ——————————————————导入数据——————————————————————
f = open('sample/lstm_mult.csv')
df = pd.read_csv(f)     # 读入股票数据
data = df.iloc[:, 1:13].values  # 取第3-10列
...
def train_lstm(batch_size=80, train_end=train_size):
    ...
    with tf.Session() as sess:
        ...
        for i in range(1000):
            ...
            print(i, loss_)
            if i % 200 == 0:
                print("保存模型：", saver.save(sess, 'module1/stock2.model', global_step=i))
...
def prediction():
    ...
    with tf.Session() as sess:
        # 参数恢复
        module_file = tf.train.latest_checkpoint('module1')
